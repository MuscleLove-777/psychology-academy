/* ============================================
   Psychology Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('psychology-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('psychology-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('psychology-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('psychology-progress', JSON.stringify(this.progress));
        localStorage.setItem('psychology-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>心理学 Academy へようこそ</h2>' +
            '<p>認知バイアスから対人心理まで、実生活に活かせる心理学を体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('psychology-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('psychology-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('psychology-progress');
            localStorage.removeItem('psychology-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
